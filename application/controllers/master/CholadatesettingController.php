...
class CholadatesettingController extends CI_Controller{
	...
	public function manage_datesetting(){
		...
		$this->load->view("master/master-choladateset-manage",$arr);	
	}
	public function remove_datesetting($enc_dset_id){
		...
		$adminrow=$this->admod->getAdminProfile($masterId);
		$temple_id=$adminrow->ad_temple;
		$dset_id=$this->encryptcode->decrypt($enc_dset_id,ENC_KEY_PASS);
		...
		if($dset_templeid==$temple_id){
			...
			if($delrecord){
				$this->session->set_flashdata('feedback',"Date removed successfully.");
				$this->session->set_flashdata('feedbackerr',"alert-success");
				redirect('master/chola-datemgmt/manage');	
			}
		}else{
			redirect('master/chola-datemgmt/manage');	
		}
	...
	public function add_datesetting(){
		...
		if($this->form_validation->run()==true){
			...
			$this->load->view('master/master-choladateset-add',$arr);	
		}
	}
