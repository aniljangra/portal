<?php
class CholadatesetModel extends CI_Model {
	...
	public function getAllDateSetting($temple_id){
			...
			$this->db->join('tb_temple','tb_temple.temple_id=tb_choladatesetting.dset_templeid','left');
			$query=$this->db->get();
			return $query->result();	
	}
	...
	public function delPerDateSetting($dset_id){
		$this->db->where('dset_id',$dset_id);
		$query=$this->db->delete('tb_choladatesetting');
		return $query;
	}
	
	public function addDateSetting($data){
		$dataInsert=array('dset_date'=>$data['dset_date'],'dset_templeid'=>$data['dset_templeid']);
		return $this->db->insert('tb_choladatesetting',$dataInsert);		
	}
}
?>